...
As
 Select *, (Select max(ID) From Nt_Order) As MaxID, (Select Top 1 Name From Nt_PaymentMethod Where Id=PaymentMethodId) As PaymentMethod, (Select Top 1 Name From Nt_ShippingMethod Where Id=ShippingMethodId) As ShippingMethod, (OrderTotal-OrderDiscount+ShippingExpense+CommissionCharge) As AmountPayable From Nt_Order As T0 Left Join (Select Id As TempID,[Name],[Phone],[Mobile],[Email],[Address],[Zip] From Nt_Customer_Consignee) As T1 On T0.CustomerConsigneeId=T1.TempID
GO
