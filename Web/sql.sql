...
GO
--Drop View 'View_Customer_Message' 
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[View_Customer_Message]'))
    Drop VIEW [dbo].[View_Customer_Message]
GO
...
GO
--drop Foreign Key 'FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer' 
IF OBJECT_ID(N'[dbo].[FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer]','F') IS NOT NULL
  ALTER TABLE [dbo].[Nt_Customer_Message_Customer_Mapping] DROP CONSTRAINT [FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer];
GO
--drop Foreign Key 'FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer_Message' 
IF OBJECT_ID(N'[dbo].[FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer_Message]','F') IS NOT NULL
  ALTER TABLE [dbo].[Nt_Customer_Message_Customer_Mapping] DROP CONSTRAINT [FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer_Message];
GO
...
IF OBJECT_ID(N'[dbo].[Nt_Customer_Message_Customer_Mapping]', 'U') IS NOT NULL
    DROP TABLE [dbo].[Nt_Customer_Message_Customer_Mapping];
GO
...
IF OBJECT_ID(N'[dbo].[Nt_CustomHtmlBlock]', 'U') IS NOT NULL
    DROP TABLE [dbo].[Nt_CustomHtmlBlock];
GO
...
GO
--Creating Table 'Nt_Customer_Message_Customer_Mapping' 
Create Table [dbo].[Nt_Customer_Message_Customer_Mapping] (
    [CustomerId] int not null,
...
GO
--Creating Table 'Nt_CustomHtmlBlock' 
Create Table [dbo].[Nt_CustomHtmlBlock] (
...
ON DELETE CASCADE ON UPDATE NO ACTION;
GO
--Creating Foreign Key 'FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer' 
ALTER TABLE [dbo].[Nt_Customer_Message_Customer_Mapping]
ADD CONSTRAINT [FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer]
FOREIGN KEY ([CustomerId]) REFERENCES [dbo].[Nt_Customer]([Id])
ON DELETE CASCADE ON UPDATE NO ACTION;
GO
--Creating Foreign Key 'FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer_Message' 
ALTER TABLE [dbo].[Nt_Customer_Message_Customer_Mapping]
ADD CONSTRAINT [FK_Nt_Customer_Message_Customer_Mapping_Nt_Customer_Message]
FOREIGN KEY ([CustomerMessageId]) REFERENCES [dbo].[Nt_Customer_Message]([Id])
ON DELETE CASCADE ON UPDATE NO ACTION;
GO
...
As
 Select *,(Select max(ID) From Nt_Article) As MaxID,0 As PreID,0 As NextID,'' As PreTitle,'' As NextTitle  From Nt_Article  Left Join (Select Id As TempID,Name As ClassName,Crumbs As ClassCrumbs,ListTemplate,DetailTemplate From Nt_Article_Class) As T  On Nt_Article.ArticleClassId=T.TempID
GO
...
GO
--Creating View 'View_Customer_Message' 
Create View [dbo].[View_Customer_Message]
As
Select *,0 As MaxID, (Select Count(0) From Nt_Customer_Message_Customer_Mapping Where CustomerMessageId=T0.Id) As TotalSent From Nt_Customer_Message As T0
GO
...
As
 Select *, (Select max(ID) From Nt_Order) As MaxID, (Select Top 1 Name From Nt_PaymentMethod Where Id=PaymentMethodId) As PaymentMethod, (Select Top 1 Name From Nt_ShippingMethod Where Id=ShippingMethodId) As ShippingMethod, (OrderTotal-OrderTotalDiscount-RefundedAmount+ShippingExpense+CommissionCharge) As AmountPayable From Nt_Order As T0 Left Join (Select Id As TempID,[Name],[Phone],[Mobile],[Email],[Address],[Zip] From Nt_Customer_Consignee) As T1 On T0.CustomerConsigneeId=T1.TempID
GO
